...
let algorithm_w lambda =
    ...
    let apply_substitution subst t =
        let rec subst_in_type binded t = match t with
            | HM_Elem v -> if ((T_map.mem v subst) && not (T_set.mem v binded)) then T_map.find v subst else t
            | HM_Arrow (t1, t2) -> HM_Arrow (subst_in_type binded t1, subst_in_type binded t2)
            (* subst just free vars *)
    ...
    let rec free_vars hm_lambda b_set = match hm_lambda with
        ...
        | HM_App (h_l1, h_l2) -> T_set.union (free_vars h_l1 b_set) (free_vars h_l2 b_set)
        | HM_Abs (name, h_l) -> free_vars h_l (T_set.add name b_set)
        | HM_Let (name, h_l1, h_l2) -> T_set.union (free_vars h_l1 b_set) (free_vars h_l2 (T_set.add name b_set)) in

    let rec free_type_vars t b_set = match t with
        | HM_Elem name -> if not (T_set.mem name b_set) then T_set.singleton name else T_set.empty
        | HM_Arrow (t1, t2) -> T_set.union (free_type_vars t1 b_set) (free_type_vars t2 b_set)
        | HM_ForAll (name, t) -> free_type_vars t (T_set.add name b_set) in

    let free_vars_in_context context =
        T_map.fold (fun key t acc -> T_set.union acc (free_type_vars t T_set.empty)) context (T_set.empty) in

    let cl_in_context t context =
        let free_in_t = free_type_vars t T_set.empty in
    ...
    try (
        ...
        (T_set.iter (fun el -> free_var_types := (T_map.add el (new_type()) !free_var_types)) (free_vars lambda T_set.empty));
